            ...
            if os.path.exists(env_path):
                with open(env_path, "r", encoding="utf-8") as f:
                    for line in f.read().splitlines():
                        ...
                        k = k.strip()
                        v = v.strip().strip("'\"")
                        if k and v and (env_file or k not in os.environ):
                            os.environ[k] = v
                return
    except Exception:
        pass
...
def parse_args():
    ...
    p.add_argument("--env-file", default=None, help="Путь к .env файлу")
    ...
    p.add_argument("--methods", nargs="+", default=["GET"], help="Порядок HTTP-методов, например: GET POST")
    ...
    p.add_argument("--host-suffix", default="")
...
def main():
    # Вычитываем --env-file из sys.argv до parse_args(),
    # чтобы env-переменные были доступны для дефолтных значений аргументов
    env_file = None
    try:
        ...
        pass
    # Загружаем .env из cwd (если есть), а затем --env-file (если указан)
    load_local_env(env_file=None)
    if env_file:
